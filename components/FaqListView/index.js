...
import c from "./faqList.module.css";
...
function FaqList(props) {
  ...
  async function loadData() {
    try {
      const res = await fetchData("/api/faq/list");
      res.success ? setFaqList(res.data) : setFaqList([]);
    } catch (err) {
      console.log(err);
      toast.error("FAQ 데이터를 불러오는 중 오류가 발생했습니다.");
    }
    setLoaded(true);
  }
      ...
      <div className="custom_container">
        ...
        {faqList.length > 0 &&
          faqList.map((faq, index) => (
            <AccordionFaq key={index} title={t(`${faq.question}`)}>
              {faq.answer}
            </AccordionFaq>
          ))}
      </div>
