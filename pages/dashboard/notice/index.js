...
const NoticeList = () => {
  ...
  useEffect(() => {
    setPermissions(cpf(session, "notice"));
  }, [session]);
  ...
  const deleteNotice = async () => {
    setIsOpen(false);
    await deleteData(`/api/notice?id=${selectedNotice}`)
      .then((data) =>
        data.success
          ? (toast.success("공지사항이 성공적으로 삭제되었습니다."), mutate())
  ...
  const filteredItems = noticeList.filter(
    (item) =>
    ...
    },
    {
      ...
      selector: (row) => (
        <div>
          ...
          {permissions.edit && (
            <Link href={`/dashboard/notice/${row._id}`}>
  ...
  return (
    <>
      {error ? (
        ...
        <div>
          <h4 className="text-center pt-3 pb-5">공지사항 관리</h4>
          <div className="d-flex justify-content-end mb-3">
            <Link href="/dashboard/notice/create">
              <button className="btn btn-primary">공지사항 등록</button>
            </Link>
          </div>
          <div className={classes.container}>
            <DataTable
              ...
              noDataComponent="등록된 공지사항이 없습니다."
            />
          </div>
          <GlobalModal
            isOpen={isOpen}
...
NoticeList.requireAuthAdmin = true;
NoticeList.dashboard = true;
