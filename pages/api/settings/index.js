 ...
 *     tags:
 *       - Settings
 *     summary: Get application settings (애플리케이션 설정 조회)
 *     description: Retrieve all application settings (모든 애플리케이션 설정 조회)
 *     security:
 *       - sessionAuth: []
 *     responses:
 *       200:
 *         description: Settings retrieved successfully (설정 조회 성공)
 *       400:
 ...
 *     tags:
 *       - Settings
 *     summary: Update application settings (애플리케이션 설정 업데이트)
 *     description: Update settings by scope - general, layout, graphics, seo, script, gateway, login, security, sitemap, robot (범위별 설정 업데이트 - 일반, 레이아웃, 그래픽, SEO, 스크립트, 결제, 로그인, 보안, 사이트맵, robots.txt)
 *     security:
 *       - sessionAuth: []
 *     parameters:
 *       - in: query
 ...
 *         schema:
 *           type: string
 *           enum: [general, layout, graphics, seo, script, gateway, login, security, sitemap, robot]
 *         description: Settings scope to update (업데이트할 설정 범위)
 *     requestBody:
 *       required: true
 *       content:
 *         multipart/form-data:
 ...
 *     responses:
 *       200:
 *         description: Settings updated successfully (설정 업데이트 성공)
 *       400:
 *         description: Bad request (잘못된 요청)
 *       403:
 *         description: Access forbidden (접근 거부)
