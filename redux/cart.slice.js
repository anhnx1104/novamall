...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    resetCart: (state, action) => {
      const { coupon, billingInfo, shippingInfo, deliveryInfo } = initialState;
      
      // If there are items with selected property, only remove selected items
      if (state.items.some(item => item.selected !== undefined)) {
        state.items = state.items.filter(item => !item.selected);
      } else {
        // If no items have a selected property, reset all items (original behavior)
